...
* **Parent to Child:** Data is passed down via **props**. For example, the parent `Cards` component passes each technology item to the child `Card` component using `<Card card="{card}"/>`.
* **Child to Parent:** The parent passes a **callback function** to the child as a prop. When an event happens inside the child (e.g., clicking the "Add to Stack" button), the child calls that function with data, like `onSelect(card)`, which updates the parent's state.
